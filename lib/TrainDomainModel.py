...
if __name__ == '__main__':
	...
	try:
		...
		if parse_config(args.c)[4] is not None :
			log_path = str(parse_config(args.c)[4])+ 'Train_Domain_Data_log.txt'
		else:
			log_path = str(os.getcwd())+"/"+ 'Train_Domain_Data_log.txt'
	except:
		parser.print_help()
		sys.exit(0)

	logger = logging.getLogger('Train_Domain_Data_log.txt')
	...
	train_lm(domain_data_path,model_path,str(args.dn),str(args.sl),str(args.tl),str(parse_config(args.c)[1]),str(parse_config(args.c)[2]))
	print('Done')
