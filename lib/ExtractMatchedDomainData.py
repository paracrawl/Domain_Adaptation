...
if __name__ == '__main__':
	...
	try:
		...
		if parse_config(args.c)[4] is not None :
			log_path = str(parse_config(args.c)[4])+ 'Extract_Matched_Domain_log.txt'
		else:
			log_path = str(os.getcwd())+"/"+ 'Extract_Matched_Domain_log.txt'
	except:
		parser.print_help()
		sys.exit(0)
	logger = logging.getLogger('Extract_Matched_Domain_log.txt')
	...
	if not os.path.exists(str(pool_data_path)):
		...
		for item_to_domain in get_file_list(scoring_path) :
			#Set the Extractor
			print("Selected Threshold: \t",float(args.est))
			...
			print(str(i) + " Items  selected from " + str(s) + " in " + str(item_to_domain))
